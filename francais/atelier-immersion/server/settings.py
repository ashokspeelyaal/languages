...
DATABASE_PATH = Path(os.environ.get("DATABASE_PATH", str(ROOT / "data" / "immersion.db"))).resolve()
...
PORT = int(os.environ.get("PORT", "15193"))
...
def parse_users(raw: str):
    """USERS=ashok:pw1,partner:pw2 → [('ashok', 'pw1'), …]"""
    out = []
    for entry in raw.split(","):
        ...
        if name.strip() and pw.strip():
            out.append((name.strip(), pw.strip()))
    return out
