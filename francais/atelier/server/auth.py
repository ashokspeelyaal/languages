...
def set_cookie(resp: Response, token: str) -> None:
    resp.set_cookie(
        ...
        httponly=True,
        samesite="lax",
        secure=False,  # set True when serving over HTTPS — Cloudflare handles that
        path="/",
    )
...
def require_user(
    request: Request,
    atelier_session: Optional[str] = Cookie(default=None),
) -> dict:
    user = lookup_session(atelier_session)
    if not user:
