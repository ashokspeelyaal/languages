...
(function () {
  ...
  function isDue(item) {
    ...
    return p.due <= window.Store.today();
  }

  function pickDue(items, max) {
    const due = items.filter(isDue);
    // Interleave: shuffle to mix categories
    shuffle(due);
    // Prioritise lower-box items (struggling ones)
    due.sort((a, b) => {
  ...
  window.SRS = { INTERVAL_DAYS, nextDueFor, isDue, pickDue, boxCounts, shuffle };
})();
