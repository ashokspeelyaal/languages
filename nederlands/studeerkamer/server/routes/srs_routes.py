...
@router.post("/review")
def review(body: dict, user=Depends(require_user)):
    ...
    XP (stored in user_kv), and streak.
    """
...
@router.post("/star")
def star(body: dict, user=Depends(require_user)):
    item_id = body.get("itemId")
    if not item_id:
...
def session_start(body: dict, user=Depends(require_user)):
